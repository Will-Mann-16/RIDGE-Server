...
module.exports.createViewToken = function(house, callback){
  callback({ success: true, token: jwt.sign({ user: {house: house, role: 4}}, secretKey)}, 200);
};
//User
      ...
      if (err) {
        ...
        getConfig(decoded.user.house, function(response, status) {
          if (response.success && status == 200) {
            callback(
              { success: true, user: decoded.user, config: response.config },
              200
            );
          } else {
...
module.exports.updateStudentLocation = function(
  ids,
  queryLocation,
  callback,
  createHistory
...
module.exports.uploadStudents = function(json, house, callback) {
  ...
  House.findOne({ _id: house }, function(err5, housedata) {
    ...
    Location.findOne({ _id: housedata.config.DEFAULT_LOCATION }, function(
      err6,
      location
    ) {
      if (err6 && success) {
  ...
  Student.findOne({ code: username.toLowerCase() }, function(err, hash) {
    ...
    if (success && hash != null) {
      bcrypt.compare(password, hash.password, function(err1, result) {
        ...
        if (result && success) {
          Student.findOne({ code: username.toLowerCase() }, function(
            err2,
            student
          ) {
            if (err2 && success) {
...
module.exports.appUpdateStudentLocation = function(
  studentID,
  locationID,
  callback,
  createHistory
...
module.exports.readCalender = function(house, callback) {
  Calender.find({ _house: house }, {}, { sort: { starttime: 1 } }, function(
    err,
    events
  ) {
    if (err) {
